...
def load_model(model):
    return model.load_state_dict(torch.load('./saved_models/deterministicmodel.pth', map_location=device))
...
while True:
    ...
    if len(sprites) > 0 and num_steps_0 == 0 and num_steps_1 == 0 and mouse_state_bool:
        ...
        theta0_std = np.std(lst_theta0)
        theta1_std = np.std(lst_theta1)
        theta0_mean = np.mean(lst_theta0)
        theta1_mean = np.mean(lst_theta1)

        print(f"\n--- Uncertainty Estimation ---")
        print(f"Theta_0 mean (radians): {theta0_mean:.4f}")
        print(f"Theta_0 std dev: {theta0_std:.4f}")
        print(f"Theta_1 mean (radians): {theta1_mean:.4f}")
        print(f"Theta_1 std dev: {theta1_std:.4f}")
        print(f"---------------------------------\n")

# Plot the uncertainty graphs
        uncertainty_graphs = helpers.make_uncertainty_plots(
             sorted(lst_theta0[:-1]), sorted(lst_theta1[:-1]), lst_theta0[-1], lst_theta1[-1]
        )

# Clear lists for next point
        del(lst_theta0[:])
        del(lst_theta1[:])
