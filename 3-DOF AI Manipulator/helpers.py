...
# Order a set while preserving sequence order
...
def make_uncertainty_plots(h, h_2, p, p2):
    ...
    plt.title("Theta 1")
    plt.plot(h, fit, '-o')
    plt.hist(h, density=True, bins=20, alpha=0.6, color='skyblue')
    plt.yticks([])
    plt.xlim((0, 6.5))
    plt.xlabel("Radians")
    ...
    plt.hist(h_2, density=True, bins=20, alpha=0.6, color='lightcoral')
    plt.yticks([])
    plt.xlim((0, 6.5))
    plt.xlabel("Radians")
    ...
    # Render to pygame surface (matplotlib ≥3.9 compatible)
