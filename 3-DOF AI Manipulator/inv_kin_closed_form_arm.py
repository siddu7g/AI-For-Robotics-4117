...
def inv_kin_2arm(x, y, link0_length, link1_length):
    ...
    # Law of Cosines check: is target reachable?
    cos_theta1 = (dist_sq - link0_length ** 2 - link1_length ** 2) / (2 * link0_length * link1_length)

    if cos_theta1 < -1 or cos_theta1 > 1:
        return -1, -1  # Unreachable

    # Compute theta1 (elbow angle)
    theta1 = np.arccos(cos_theta1)

    # Compute theta0 (shoulder angle)
    k1 = link0_length + link1_length * np.cos(theta1)
    k2 = link1_length * np.sin(theta1)
    theta0 = np.arctan2(y, x) - np.arctan2(k2, k1)
