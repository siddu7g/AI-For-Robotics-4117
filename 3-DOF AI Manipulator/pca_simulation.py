...
def calculate_pca(data, k=2):
    ...
    # Columnwise mean - This computes the mean value of each column in the data matrix.
    data_mean = torch.mean(data, dim=0, keepdim=True)
    data = data - data_mean # Centers the data around the origin
    # SVD breaks the centered data matrix into three parts U, S, and V
    # V gives the principal directions (the axes along which variance is maximum)
    # S shows how much variance each direction captures.
    U, S, V = torch.svd(data)

    return U[:, :k], data_mean[0][0], data_mean[0][1]
...
while True:
    ...
    if current_state ==3 and num_steps_0 == 0 and num_steps_1 == 0:
        ...
        print("Principal Component Vectors:", m_0, " ", m_1)
        current_state = 4
