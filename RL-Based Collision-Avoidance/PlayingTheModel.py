...
width = 600  # Width Of The Game Windowpyt
...
if __name__ == "__main__":
    ...
    for idx, (x0, y0, th0) in enumerate(poses, start=1):
        ...
        for step in range(max_steps):
            # goal check
            if abs(env.Bot.position[0] - GOAL[0]) <= tol and abs(env.Bot.position[1] - GOAL[1]) <= tol:
                print(f"Pose {idx}: MISSION COMPLETE at step {step}")
                break

            # Handle crash
            if env.DetectCrash > 0:
                side = env.DetectCrash  # 3 = left, 4 = right
                for _ in range(10):
                    env._step(side)
            else:
                env._step(5)
